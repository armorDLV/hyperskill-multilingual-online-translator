...
def valid_inputs() -> bool:
    if args.source not in language_dict.values():
        print(f"Sorry, the program doesn't support {args.source}")
        return False
    if args.target not in language_dict.values():
        print(f"Sorry, the program doesn't support {args.target}")
        return False
    url = 'https://context.reverso.net/translation/'
    for x in language_dict.values():
        if x != args.target:
            url += f'{args.source}-{x}/{args.word}'
            break
...
def print_translations(src: str, trg: str, wrd: str, num: int = 1):
    ...
    if (response := s.get(url)).status_code != 200:
